...
class OrderServiceBrokerOutcomeAdapter(
    private val brokerOrdersServiceWrapper: SyncStubWrapper<OrdersServiceBlockingStub>,
    ...
    override fun postBestPriceBuyOrder(command: PostBestPriceBuyOrderCommand): Either<BrokerIntegrationError, PostOrderResponse> =
        either {
            ...
            }.mapLeft { BrokerIntegrationError.BestPriceBuyOrderExecutionError }
                .bind()
    ...
    override fun postBestPriceSellOrder(command: PostBestPriceSellOrderCommand): Either<BrokerIntegrationError, PostOrderResponse> =
        either {
                ...
                log.error(errorMessage, ex)
                notificationPort.sendNotification(SendNotificationCommand(errorMessage))
            }.mapLeft { BrokerIntegrationError.BestPriceSellOrderExecutionError }
                .bind()
        }.map(brokerOutcomeAdapterMapper::map)
            .onRight { postOrderResponse ->
                notificationPort.sendNotification(
    ...
    override fun getMaxLots(command: GetMaxLotsCommand): Either<BrokerIntegrationError, GetMaxLotsResponse> =
        either {
            catch {
                brokerOrdersServiceWrapper.callSyncMethod { stub ->
                    stub.getMaxLots(
                        GetMaxLotsRequest.newBuilder()
                            .setInstrumentId(command.instrument.id)
                            .setAccountId(userServiceBrokerOutcomeAdapter.getTradingAccountId().bind())
                            .build()
                    )
                }
            }.onLeft { ex -> log.error("An error has been occurred while getting max lots", ex) }
                .mapLeft { BrokerIntegrationError.GetMaxLotsError }
                .bind()
        }.map(brokerOutcomeAdapterMapper::map)
